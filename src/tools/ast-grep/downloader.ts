...
export function getCacheDir(): string {
  if (process.platform === 'win32') {
    ...
    return join(base, 'oh-my-opencode-mal', 'bin');
  }
...
export async function downloadAstGrep(
  ...
  if (!platformInfo) {
    console.error(
      `[oh-my-opencode-mal] Unsupported platform for ast-grep: ${platformKey}`,
    );
    return null;
  }
  ...
  console.log(`[oh-my-opencode-mal] Downloading ast-grep binary...`);
  ...
  try {
    ...
    console.log(`[oh-my-opencode-mal] ast-grep binary ready.`);

    return binaryPath;
  } catch (err) {
    console.error(
      `[oh-my-opencode-mal] Failed to download ast-grep: ${err instanceof Error ? err.message : err}`,
    );
    return null;
  }
}
